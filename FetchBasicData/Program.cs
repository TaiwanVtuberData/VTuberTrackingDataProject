...
class Program
{
    ...
    static void MainProcess(string[] args)
    {
        ...
        Dictionary<VTuberId, YouTubeData> dictYouTube = GenerateYouTubeDataDict(
            trackList,
            FileUtility.GetSingleLineFromFile(YouTubeApiKeyPath)
        );
    ...
    static void WriteBasicData(
        Dictionary<VTuberId, YouTubeData> dictYouTube,
        Dictionary<VTuberId, TwitchData> dictTwitch,
    ...
    )
    {
        ...
        using StreamWriter writer =
            new($"{fileDir}/basic-data_{currentDateTime:yyyy-MM-dd-HH-mm-ss}.csv");
    ...
    // Key: VTuber ID
    static Dictionary<VTuberId, VTuberBasicData> MergeDictionary(
        Dictionary<VTuberId, YouTubeData> mainDict,
        Dictionary<VTuberId, TwitchData> minorDict
    )
    {
        ...
        foreach (KeyValuePair<VTuberId, YouTubeData> pair in mainDict)
        {
            ...
            if (minorDict.TryGetValue(VTuberId, out TwitchData value))
            {
                rDict.Add(
                    VTuberId,
                    new VTuberBasicData(Id: VTuberId, YouTube: youTubeData, Twitch: value)
                );
            }
            else
            {
                rDict.Add(
                    VTuberId,
                    new VTuberBasicData(Id: VTuberId, YouTube: youTubeData, Twitch: null)
                );
            }
        }
        ...
        foreach (KeyValuePair<VTuberId, TwitchData> pair in minorDict)
        {
            ...
            if (!rDict.ContainsKey(VTuberId))
            {
                rDict.Add(
                    VTuberId,
                    new VTuberBasicData(Id: VTuberId, YouTube: null, Twitch: twitchData)
                );
            }
        }
    ...
    static Dictionary<VTuberId, YouTubeData> GenerateYouTubeDataDict(
        TrackList trackList,
        string apiKey
    )
    {
        Dictionary<string, VTuberId> dictChannelIdVtuberId = GenerateYouTubeIdVTtuberIdDict(
            trackList
        );
        List<string> lstIdStringList = Generate50IdsStringList(dictChannelIdVtuberId.Keys.ToList());
        // initialize capacity
        Dictionary<VTuberId, YouTubeData> dictVTuberIdThumbnailUrl =
            new(dictChannelIdVtuberId.Count);

        YouTubeService youtubeService =
            new(new BaseClientService.Initializer() { ApiKey = apiKey });
        foreach (string idStringList in lstIdStringList)
        {
            ChannelsResource.ListRequest channelListRequest = youtubeService.Channels.List(
                "snippet, statistics"
            );
            channelListRequest.Id = idStringList;
            channelListRequest.MaxResults = 50;

            Google.Apis.YouTube.v3.Data.ChannelListResponse? channellistItemsListResponse =
                ExecuteYouTubeThrowableWithRetry(() => channelListRequest.Execute());
            ...
            foreach (
                Google.Apis.YouTube.v3.Data.Channel channelItem in channellistItemsListResponse.Items
            )
            {
                ...
                if (
                    hasId
                    && VTuberId is not null
                    && !dictVTuberIdThumbnailUrl.ContainsKey(VTuberId)
                )
                {
                    dictVTuberIdThumbnailUrl.Add(
                        VTuberId,
                        new YouTubeData(
    ...
    static Dictionary<VTuberId, TwitchData> GenerateTwitchDataDict(
        TrackList trackList,
        TwitchAPI api
    )
    {
        Dictionary<string, VTuberId> dictChannelIdVtuberId = GenerateTwitchIdVTuberIdDict(
            trackList
        );
        List<List<string>> lstIdStringList = Generate100IdsStringListList(
            dictChannelIdVtuberId.Keys.ToList()
        );
        // initialize capacity
        ...
        {
            Dictionary<string, TwitchData> dictTwitch = GetTwitchIdThumbnailUrlDict(
                idStringList,
                api
            );
    ...
    static Dictionary<string, TwitchData> GetTwitchIdThumbnailUrlDict(
        List<string> userIdList,
        TwitchAPI api
    )
    {
        TwitchLib.Api.Helix.Models.Users.GetUsers.GetUsersResponse? getUsersResponse = api.GetUsers(
            userIdList,
            log
        );
        ...
        foreach (TwitchLib.Api.Helix.Models.Users.GetUsers.User user in getUsersResponse.Users)
        {
            ...
            rDict.Add(
                user.Id,
                new TwitchData(
                    FollowerCount: nullableFollowerCount ?? 0ul,
    ...
    static T? ExecuteYouTubeThrowableWithRetry<T>(Func<T> func)
        where T : class?
    {
        int RETRY_TIME = 10;
