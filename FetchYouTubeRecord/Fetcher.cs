...
public class Fetcher
{
    ...
    public Fetcher(string ApiKey, DateTimeOffset currentTime)
    {
        ...
        youtubeService = new YouTubeService(
            new BaseClientService.Initializer() { ApiKey = this.ApiKey }
        );
    ...
    public (
        ImmutableDictionary<YouTubeChannelId, YouTubeRecord>,
        TopVideosList,
        LiveVideosList
    ) GetAll(IImmutableList<YouTubeChannelId> lstChannelId)
    {
        ImmutableDictionary<YouTubeChannelId, Google.Apis.YouTube.v3.Data.Channel> dictChannelInfo =
            GetChannelInfoList(lstChannelId);

        ImmutableDictionary<YouTubeChannelId, YouTubeRecord.BasicRecord> dictBasicRecord =
            GetChannelBasicRecord(dictChannelInfo);

        ChannelRecentViewRecord channelRecentViewRecord = GetChannelRecentViewRecord(
            dictChannelInfo
        );

        ImmutableDictionary<YouTubeChannelId, YouTubeRecord> rDict = CreateYouTubeRecord(
            dictBasicRecord,
            channelRecentViewRecord.DictRecentRecordTuple
        );

        return (
            rDict,
            channelRecentViewRecord.TopVideosList,
            channelRecentViewRecord.LiveVideosList
        );
    }
    ...
    private static ImmutableDictionary<YouTubeChannelId, YouTubeRecord> CreateYouTubeRecord(
        ...
        IImmutableDictionary<
            YouTubeChannelId,
            YouTubeRecord.RecentRecordTuple
        > dictRecentRecordTuple
    )
    {
        ...
        foreach (
            KeyValuePair<
                YouTubeChannelId,
                YouTubeRecord.BasicRecord
            > keyValuePair in dictBasicRecord
        )
        {
            YouTubeChannelId channelId = keyValuePair.Key;
            YouTubeRecord.BasicRecord basicRecord = keyValuePair.Value;
            dictRecentRecordTuple.TryGetValue(
                channelId,
                out YouTubeRecord.RecentRecordTuple? recentRecordTuple
            );
    ...
    private ImmutableDictionary<
        YouTubeChannelId,
        Google.Apis.YouTube.v3.Data.Channel
    > GetChannelInfoList(IImmutableList<YouTubeChannelId> lstChannelId)
    {
        Dictionary<YouTubeChannelId, Google.Apis.YouTube.v3.Data.Channel> rDict =
            new(lstChannelId.Count);

        // When querying multiple channel statistics, only 50 videos at a time is allowed
        ImmutableList<IdRequstString> lstIdRequest = Generate50IdsStringList(
            lstChannelId.Map(e => e.Value).ToImmutableList()
        );

        foreach (IdRequstString idRequest in lstIdRequest)
        {
            ImmutableList<Google.Apis.YouTube.v3.Data.Channel>? lstChannelInfo =
                GetChannelStatisticsResponse(idRequest);
    ...
    private ImmutableList<Google.Apis.YouTube.v3.Data.Channel>? GetChannelStatisticsResponse(
        IdRequstString idRequestString
    )
    {
        ChannelsResource.ListRequest channelsListRequest = youtubeService.Channels.List(
            "statistics, contentDetails"
        );
        channelsListRequest.Id = idRequestString.Value;
        channelsListRequest.MaxResults = 50;

        Google.Apis.YouTube.v3.Data.ChannelListResponse? channelsListResponse =
            ExecuteYouTubeThrowableWithRetry(() => channelsListRequest.Execute());

        // channelsListResponse.Items is actually nullable
    ...
    private ImmutableDictionary<YouTubeVideoId, DateTimeOffset> GetChannelRecentVideoList(
        Google.Apis.YouTube.v3.Data.Channel channelInfo
    )
    {
        ...
        while (nextPageToken != null)
        {
            PlaylistItemsResource.ListRequest playlistItemsListRequest =
                youtubeService.PlaylistItems.List("snippet,status");
            ...
            Google.Apis.YouTube.v3.Data.PlaylistItemListResponse? playlistItemsListResponse =
                ExecuteYouTubeThrowableWithRetry(() => playlistItemsListRequest.Execute());
            ...
            foreach (
                Google.Apis.YouTube.v3.Data.PlaylistItem playlistItem in playlistItemsListResponse.Items
            )
            {
    ...
    )
    {
        ...
        foreach (
            KeyValuePair<
                YouTubeChannelId,
                Google.Apis.YouTube.v3.Data.Channel
            > keyValuePair in dictChannelInfo
        )
        {
            YouTubeChannelId channelId = keyValuePair.Key;
            Google.Apis.YouTube.v3.Data.Channel channelInfo = keyValuePair.Value;

            ImmutableDictionary<YouTubeVideoId, DateTimeOffset> dictVideoIdAndTime =
                GetChannelRecentVideoList(channelInfo);

            // When querying multiple video statistics, only 50 videos at a time is allowed
            ImmutableList<IdRequstString> lstIdRequest = Generate50IdsStringList(
                dictVideoIdAndTime.Keys.Map(e => e.Value).ToImmutableList()
            );
            ...
            {
                VideosResource.ListRequest videosListRequest = youtubeService.Videos.List(
                    "id,snippet,statistics,liveStreamingDetails"
                );
                videosListRequest.Id = idRequst.Value;

                Google.Apis.YouTube.v3.Data.VideoListResponse? videoListResponse =
                    ExecuteYouTubeThrowableWithRetry(() => videosListRequest.Execute());
                ...
                foreach (Google.Apis.YouTube.v3.Data.Video video in videoListResponse.Items)
                {
                    ...
                    {
                        lstTotalViewCount.Add(
                            new(
                                publishTime ?? DateTimeOffset.UnixEpoch,
                                new YouTubeVideoId(video.Id),
                                viewCount.Value
                            )
                        );
                        ...
                        if (isLivestream)
                        {
                            DateTimeOffset? startTime = video
                                .LiveStreamingDetails
                                ?.ActualStartTimeDateTimeOffset;
                            DateTimeOffset? endTime = video
                                .LiveStreamingDetails
                    ...
                    if (LiveVideoTypeConvert.IsLiveVideoType(video.Snippet.LiveBroadcastContent))
                    {
                        ...
                        rLiveVideosList.Add(
                            new LiveVideoInformation
                            {
                                ...
                                VideoType = LiveVideoTypeConvert.FromString(
            ...
            YouTubeRecord.RecentRecord recentTotalRecord = CreateRecentRecord(
                lstTotalViewCount.ToImmutableList(),
                CurrentTime
            );
            YouTubeRecord.RecentRecord recentLivestreamRecord = CreateRecentRecord(
                lstLivestreamViewCount.ToImmutableList(),
                CurrentTime
            );
            YouTubeRecord.RecentRecord recentVideoRecord = CreateRecentRecord(
                lstVideoViewCount.ToImmutableList(),
                CurrentTime
            );

            rDict.Add(
                channelId,
                new YouTubeRecord.RecentRecordTuple(
    ...
    )
    {
        ...
        {
            ImmutableList<Tuple<DateTimeOffset, string, ulong>> lstIdViewCountTemp = lstIdViewCount
                .Map(e => new Tuple<DateTimeOffset, string, ulong>(e.Item1, e.Item2.Value, e.Item3))
                .ToImmutableList();

            medianViews = NumericUtility.GetMedian(lstIdViewCountTemp);
            popularity = (ulong)
                NumericUtility.GetPopularity(lstIdViewCountTemp, currentTime.UtcDateTime);
            Tuple<DateTimeOffset, string, ulong> largest = NumericUtility.GetLargest(
                lstIdViewCountTemp
            );
    ...
    private static ImmutableList<IdRequstString> Generate50IdsStringList(
        IImmutableList<string> KeyList
    )
    {
    ...
    private static T? ExecuteYouTubeThrowableWithRetry<T>(Func<T> func)
        where T : class?
    {
        int RETRY_TIME = 10;
        ...
        for (int i = 0; i < RETRY_TIME; i++)
        {
            ...
            {
                log.Warn(
                    $"Failed to execute {func.Method.Name}. {i} tries. Retry after {RETRY_DELAY.TotalSeconds} seconds."
                );
